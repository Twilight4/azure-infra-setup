...
This repo builds a hub-and-spoke network, an App Service (Free F1), and an Azure SQL Basic DB (250 MB) for learning. It intentionally avoids paid services (no VMSS, no Azure Firewall, no Private Endpoints).

See =staging/= for environment entrypoint and =modules/= for reusable components.

Notes: this setup is *for learning* and *not production-ready*. Do not expose secrets in VCS.
...
│ │ └── outputs.tf
│ └── db/
│ ├── main.tf # Azure Database for Azure MSSQL Server (Free 250 MB Database)
│ ├── variables.tf
│ └── outputs.tf
...
- Modular layout with these modules:
  ...
  + *db* — managed DB (Azure MSSQL Server) with private access and firewall rule example
...
- *Data (db) layer* in its own spoke containing managed DB services (Azure Database Azure MSSQL) with =public_network_access_enabled = false= so only private connectivity is allowed.
...
#+begin_src shell
# Initialize terraform from /staging directory
cd staging/
